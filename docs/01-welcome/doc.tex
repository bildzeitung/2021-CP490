...
\begin{homeworkSubsection}{Content Service}
	...
	\begin{figure}
		...
		\begin{tikzpicture}
			...
			\pic [left = 1.25cm of R] {entity={RA}{Attributes}{%
									id \\
									room\_id \\
									title \\
									value \\
	...
	The Content service contains all of the game assets -- text, descriptions, and properties for rooms and items. Figure \ref{fig:content-db} is a basic data model. Items can have multiple properties and aliases. An important attribute for rooms is their exits. The direction (for example, \texttt{NORTH}) would be the attribute \texttt{title} and the title of the destination room would be the \texttt{value}.
\end{homeworkSubsection}
