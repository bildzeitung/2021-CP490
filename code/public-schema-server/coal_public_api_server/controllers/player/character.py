...
def search(player_id):
    try:
        rv = requests.get(f"{current_app.config['PLAYER_SERVER_URL']}/player/{player_id}/character")
        rv.raise_for_status()
...
def delete(player_id, character_id):
  try:
    rv = requests.delete(f"{current_app.config['PLAYER_SERVER_URL']}/player/{player_id}/character/{character_id}")
  except Exception as e:
    abort(500, f"Could not contact player server: {str(e)}")
